...
class FlattenDWCShapes(Transformation):
    def apply(self, model):
        for node in model.graph.node:
            producer = model.find_producer(node.input[0])
            consumer = model.find_consumer(node.output[0])
            if (node.op_type == 'StreamingDataWidthConverter' and
                producer.op_type == 'Thresholding_hls' and
                consumer.op_type == 'MVAU_hls'):
                shape = get_node_attr_value(node, 'shape')
...
class InsertAndSetFIFODepthsCustom(Transformation):
    ...
    def apply(self, model):
        ...
        model = model.transform(FlattenDWCShapes())
